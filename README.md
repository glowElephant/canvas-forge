...
5. 프레임 안에 **뭐든 올려라** — 텍스트·그림은 물론, 이미지 드롭(png/jpg/gif/webp/svg), 텍스트 파일 드롭(json/yml/md/csv 등 → 📄 파일 카드), URL 붙여넣기(제목·썸네일 자동). Claude가 전부 읽는다.
...
✅ **MVP3a 동작** — 멀티모달 읽기: 이미지 원본 개별 전달·텍스트 파일 카드·링크 unfurl+본문 읽기. 실 브라우저 검증(테스트 29개 통과).

로드맵: ~~MVP1~~ → ~~MVP2~~ → ~~MVP3a~~ → MVP3b(PDF·iframe 임베드·보드 내 북마크) → MVP3c(영상 프레임 추출·크로스 프로젝트 북마크).
