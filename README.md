...
7. **채팅 (커서 + 패널, 같은 스트림)** — `/` 키로 커서 채팅: 엔터마다 한 줄씩 커서 아래에 쌓였다가 몇 초 뒤 페이드아웃. 우측 **💬 채팅 패널**에는 같은 대화가 **계속 누적**되고(서버가 `.board/chat.json`에 보관 — 늦게 들어와도 이전 대화 보임), 패널에서 보내도 커서 말풍선으로 뜬다. 패널은 접기/펼치기(접힌 동안 안 읽은 수 표시)·드래그 이동 가능.
...
✅ **협업 UX** — 커서 채팅(`/`, 줄별 페이드아웃, 실시간 릴레이), 영역 ★ 지정(Claude 우선 처리), 작성 시간순 읽기(`[HH:MM 작성자]` — 논의 흐름 이해), 드래그 패널(테스트 34개 통과).
