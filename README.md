...
3. Claude Code에 MCP 등록:
   ...
   ```
4. 보드에서 **프레임**을 그리고 제목을 단다(= 한 영역, 단축키 **F**). 프레임 안에 텍스트·그림을 배치. **영역 패널**(드래그로 이동 가능)에서 영역 클릭 이동·📍 위치 핀, 그리고 **🤖 버튼으로 "Claude가 볼 영역" ★ 지정**.
5. 프레임 안에 **뭐든 올려라** — 텍스트·그림은 물론, 이미지 드롭(png/jpg/gif/webp/svg), 텍스트 파일·**PDF** 드롭(→ 📄 파일 카드), URL 붙여넣기(북마크: 제목·썸네일 자동), 메뉴의 임베드 삽입으로 **웹페이지 iframe**(사이트가 프레이밍을 막으면 빈 화면 — 그땐 북마크), **영상**(mp4/webm). Claude가 전부 읽는다.
...
7. **채팅** — 우측 **💬 채팅 패널**에서 메시지를 보내면 ① 패널에 **계속 누적**되고(서버가 `.board/chat.json`에 보관 — 늦게 들어와도 이전 대화 보임) ② 보낸 사람 **커서 위에 말풍선**으로 떠서 몇 초 뒤 페이드아웃. 패널은 접기/펼치기(접힌 동안 안 읽은 수 표시)·드래그 이동 가능. (※ `/` 커서 입력은 입력 신뢰성 문제로 보류 — 입력은 패널로 일원화, 말풍선 표시는 유지)
8. Claude Code 세션에서 호출: "보드 봐줘"(★ 지정 영역 우선) 또는 "'결제 화면' 영역 읽어줘" → `read_area`가 영역을 **텍스트(작성 시간순, `[14:05 게스트]` 라벨) + 스크린샷 + 이미지 원본들 + 파일 내용 + PDF + 링크 본문 + 영상 태그 시점 프레임**으로 멀티모달로 읽음 → `post_card`로 "이거 맞나요" 카드를 띄우면 **모든 참여자 화면에 실시간 반영** → 방장이 승인하면 Claude Code 기본 도구로 실제 빌드. Claude는 논의를 시간 흐름으로 이해한다(나중 의견이 앞 의견을 뒤집으면 최신 우선).
...
✅ **MVP3c(영상) 동작** — 영상 댓글+시간 태그 큐레이션: 사람이 댓글로 짚은 시점만 Claude가 프레임+맥락으로 분석(ffmpeg 불필요 — 브라우저 캡처). 헤드리스 검증: 태그 시점 프레임 픽셀 일치 확인.
✅ **협업 UX** — 채팅 패널(누적·히스토리 영속·커서 말풍선 표시), 영역 ★ 지정(Claude 우선 처리), 작성 시간순 읽기(`[HH:MM 작성자]` — 논의 흐름 이해), 드래그 패널(테스트 34개 통과).
